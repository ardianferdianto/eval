...
$html .='<br pagebreak="true" /><p>Kunci Jawaban</p>';

foreach ($quizz['Question'] as $r){
	$no2 ++;
	$jawaban_pg=$r['answer_true'];
	$jawaban_essay=$r['answer2'];
	$html .= '<tr>';
	if($r['tipe'] == 1){
		switch ($jawaban_pg) {
			case '1':
		...
		$html .= '<td>'.$no2.'.&nbsp;&nbsp;&nbsp;&nbsp;'.$jawaban_pg.'</td>';

	}else{
		$html .= '<td>'.$no2.'.&nbsp;&nbsp;&nbsp;&nbsp;'.$jawaban_essay.'</td>';
	}
	$html .= '</tr>';
